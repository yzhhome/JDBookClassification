...
def train(config, model, train_iter, dev_iter, test_iter):
    ...
    # 大写的是CNN, DPCNN, RCNN, RNN_ATT, RNN, TRANSFORMER模型
    ...
    # 小写的是bert, roberta, xlnet模型
    else: 
        # 使用bert的AdamW优化器
        param_optimizer = list(model.named_parameters())

        #Layer Normaliztion和bias的参数不需要进行权重衰减
        ...
        #     num_warmup_steps=0,
        #     num_training_steps=(
        #     (len(train_iter) * config.num_epochs) // 
    ...
    for epoch in range(config.num_epochs):
        ...
        for i, (trains, mask, tokens, labels) in tqdm(enumerate(train_iter)):
            ...
            if total_batch % 1000 == 0 and total_batch != 0:                
                ...
                logger.info(msg.format(total_batch, loss.item(), train_acc, 
            ...
            # 离上次最好的验证集损失超过设定的batch没下降就结束训练
            if total_batch - last_improve > config.require_improvement:                
                logger.info("超过 {} 次训练验证集损失不再下降，停止训练..."
                    .format(config.require_improvement))
                flag = True
                break
        if flag:
            break
