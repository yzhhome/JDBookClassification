...
from src.utils.mldata import MLData
from src.utils import config
from src.utils.config import root_path
from src.utils.tools import (Grid_Train_model, bayes_parameter_opt_lgb,
                             query_cut, create_logger, concate_data, get_score)
...
logger = create_logger(config.log_dir + 'ml_model.log')
...
class Models(object):
    def __init__(self, 
        ...
        # 加载bert预训练模型 
        self.bert_tokenizer = BertTokenizer.from_pretrained(config.root_path + '/model/bert')
        self.bert = BertModel.from_pretrained(config.root_path + '/model/bert')

        # 初始化数据处理类
        self.ml_data = MLData(debug_model=True, train_mode=train_mode)
        ...
        if not train_mode:
            ...
            # 如果feature_engineer, 则使用lightgbm 进行训练
            if feature_engineer:
    ...
    def feature_engineer(self):
        ...
        train['lda'] = list(
            map(lambda doc: get_lda_features(self.ml_data.em.lda, doc), train['bow']))
        test['lda'] = list(
            map(lambda doc: get_lda_features(self.ml_data.em.lda, doc), test['bow']))
        ...
        logger.info("concate data")

        #  将所有的特征拼接到一起
        train = concate_data(train, train_tfidf, train_ae)
        train = concate_data(test, test_tfidf, test_ae)
    ...
    def model_select(self,
        ...
        """
        using different embedding feature to train common ML models
        """
        # 对比tfidf word2vec fasttext等词向量以及常见机器学习模型的效果
    ...
    def process(self, title, desc):
        ...
        print("concate data")
        df['labelIndex'] = 1
        df = concate_data(df, df_tfidf, df_ae)
